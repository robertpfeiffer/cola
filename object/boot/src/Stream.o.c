...
# define GC_add_roots _local_gc_addRoots
  ...
  _method(v_Stream, s__5fsizeof, Stream___5fsizeof);
  _method(v_Stream, s__5fdebugName, Stream___5fdebugName);
  _id_export("Stream", v_Stream);
  l_1= _send(s_size_5f_value_5f_, v_ImmutableString, 4, "size");
  _method(v_Stream, s_size, Stream__size);
  l_2= _send(s_size_5f_value_5f_, v_ImmutableString, 8, "nextPut:");
  _method(v_Stream, s_nextPut_, Stream__nextPut_);
  _method(v_Stream, s_nextPutAll_, Stream__nextPutAll_);
  _method(v_Stream, s_next_, Stream__next_);
  _method(v_Stream, s_print_, Stream__print_);
  l_5= _send(s_value_5f_, v_SmallInteger, 0);
  _method(v_Stream, s_isEmpty, Stream__isEmpty);
  _method(v_Stream, s_notEmpty, Stream__notEmpty);
  _leave();
}
